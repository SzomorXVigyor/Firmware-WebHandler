// Updated main.js with new features and endpoints
...
function setupEventListeners() {
	...
	if (refreshBtn) {
		refreshBtn.addEventListener("click", () => {
			...
			Promise.all([loadDevices(), loadFirmwares(), loadFirmwareStats(), checkServerHealth()]).finally(() => {
...
function handleKeyboardShortcuts(event) {
	...
	// Ctrl/Cmd + U: Focus upload (if logged in)
	if ((event.ctrlKey || event.metaKey) && event.key === "u" && authToken) {
		event.preventDefault();
	...
	// Ctrl/Cmd + L: Focus device filter
	if ((event.ctrlKey || event.metaKey) && event.key === "l") {
		event.preventDefault();
		document.getElementById("deviceFilter").focus();
	}
}
...
function toggleView() {
	...
	// Reload firmwares with new view
	loadFirmwares();
}
...
function updateAuthUI() {
	...
	if (authToken) {
		...
		const editButtons = document.querySelectorAll('.btn-outline-warning[title="Edit"]');
		const deleteButtons = document.querySelectorAll('.btn-outline-danger[title="Delete"]');
...
function displayServerStatus(isHealthy, healthData) {
	// Add a small status indicator to the navbar
	...
	if (!statusIndicator) {
		...
		statusIndicator.className = "me-3";
		navbar.insertBefore(statusIndicator, navbar.querySelector(".navbar-nav"));
	}
...
async function loadFirmwareStats() {
	try {
		...
		if (response.ok) {
			firmwareStats = stats;
			displayFirmwareStats(stats);
		}
	} catch (error) {
		console.error("Error loading firmware stats:", error);
	}
}

function displayFirmwareStats(stats) {
	// Add stats card before the device filter
	const deviceFilterCard = document.querySelector(".card.mb-4");
	...
	if (!statsCard) {
		...
		deviceFilterCard.parentNode.insertBefore(statsCard, deviceFilterCard);
	}
            ...
            <div class="row text-center">
                <div class="col-md-3">
                    <div class="p-3">
                        ...
                        <h4 class="mb-0">${stats.totalFirmwares || 0}</h4>
                        <small class="text-muted">Total Firmwares</small>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="p-3">
                        <i class="fas fa-layer-group fa-2x text-success mb-2"></i>
                        <h4 class="mb-0">${stats.deviceTypes || 0}</h4>
                ...
                <div class="col-md-3">
                    <div class="p-3">
                        ...
                        <h4 class="mb-0">${formatFileSize(stats.totalSize || 0)}</h4>
                ...
                <div class="col-md-3">
                    <div class="p-3">
                        ...
                        <h4 class="mb-0">${stats.latestUpload ? formatDate(stats.latestUpload) : "N/A"}</h4>
...
function displayFirmwaresGridView(grouped) {
	...
	container.innerHTML = Object.keys(grouped)
		.sort() // Sort device types alphabetically
		.map((deviceType) => {
            ...
            <td>
                <div class="btn-group" role="group">
                    <a href="/api/firmware/${firmware.id}/download" class="btn btn-outline-primary btn-sm px-3" title="Download">
                        ...
                        <button class="btn btn-outline-warning btn-sm px-2" ${authToken ? `style="display: inline-block";` : `style="display: none";`} onclick="editFirmware('${
		firmware.id
	}')" title="Edit">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button class="btn btn-outline-danger btn-sm px-2" ${authToken ? `style="display: inline-block";` : `style="display: none";`} onclick="deleteFirmware('${
		firmware.id
	}')" title="Delete">
...
function showEditFirmwareModal(firmware) {
	...
	// Populate form with current firmware data
...
async function deleteFirmware(firmwareId) {
	if (!confirm("Are you sure you want to delete this firmware? This action cannot be undone.")) {
		return;
	}
